...
import static jiankun.com.xiaoyou.base.InitAppiumXiaoYou.appPackage;

/** 
 * @author 作者 : lishuang 
 * @version 创建时间：2017年2月15日 下午3:13:07 
 * 类说明 页面UI获取定位父类，供给Page层使用
 */
...
public class PageAppium {
    ...
    private static int WAIT_TIME = 5;    //默认的等待控件时间



    public PageAppium(AndroidDriver<WebElement> androidDriver) {
    ...
    public boolean isIdElementExist(String id,int timeOut, boolean isShow) {
    	System.out.println("输出resource-id:" + appPackage + ":id/" + id);
    ...
    public AndroidElement waitAutoByClassName(String className) {
        return waitAutoByClassName(className, WAIT_TIME);
    }
    
    public AndroidElement waitAutoByClassName(String className, int time) {
        return waitAuto(By.className(className), time);
    }
     ...
     * @return
     */
    public List<WebElement> getManyElementByClassName(String className, int num) {
        List<WebElement> textFieldsList = driver.findElementsByClassName(className);
     ...
     * @return
     */
    public AndroidElement getListOneElementByClassName(String className,int num){
        if(driver != null){
            try{
//            	System.out.println(driver.findElements(By.className(className)).get(num).getText());
                return (AndroidElement) driver.findElements(By.className(className)).get(num);
            }catch (Exception e){
                print("getListOneElementByclassName找不到第" + num + "个控件" + className);
                return null;
            }
        }else{
            print("getListOneElementByclassName:" + className + " 时候driver为空");
            return null;
        }
    }
     ...
     * @return
     */
    public List<WebElement> getManyElementById(String id, int num) {
        if(driver != null){
            List<WebElement> textFieldsList = driver.findElementsById(id);
            List<WebElement> list = new ArrayList<WebElement>();
            try{
                for(int i=0; i<num; i++){
                    list.add(textFieldsList.get(i));
    ...
    public AndroidElement getListOneElementById(String id,int num){
        if(driver != null){
            try{
                return (AndroidElement) driver.findElementsById(BaseAppium.appPackage + ":id/" + id).get(num);
            }catch (Exception e){
