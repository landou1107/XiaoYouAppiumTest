...
/** 
 * @author 作者 : lishuang 
 * @version 创建时间：2017年2月15日 下午3:40:21 
 * 类说明 
 */
@Listeners({jiankun.com.xiaoyou.base.AssertionListener.class})
public class XiaoYouBaseAppium {
    //调试设备名字
    ...
    public static int newCommandTimeout = 720;
...
//    public AndroidDriver<AndroidElement> driver = null;
  public AndroidDriver<WebElement> driver = null;
    ...
    //构造方法
    public XiaoYouBaseAppium() {
        this(new XiaoYouBuilder());
    }

    public XiaoYouBaseAppium(XiaoYouBuilder xiaoYouBuilder) {
        print("基类初始化!");
        ...
        platformVersion = xiaoYouBuilder.platformVersion;
...
//    @BeforeTest
    public void beforeSuite() throws MalformedURLException {
        ...
        print("属性步骤5-- XiaoYouBaseAppium界面的udid：" + udid);
        //设置浏览器
        capabilities.setCapability(CapabilityType.BROWSER_NAME, browserName);
               
        //启动Driver
        print("属性步骤5-- XiaoYouBaseAppium界面的port：" + port);
...
//            AndroidElement ae = driver.findElement(By.name(name));
            WebElement ae = driver.findElement(By.name(name));
            if (hasShow) {
                return ae.isDisplayed();
            } else return ae != null;
        } catch (NoSuchElementException e) {
...
//        AndroidElement ae = findById(id);
    ...
    public AndroidElement waitAuto(final By by, int time) {
        try {
            return (AndroidElement) new AndroidDriverWait(driver, time * 1000)
